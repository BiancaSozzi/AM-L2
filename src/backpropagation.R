...
has.satisfied.condition <- function(errors, eps, max.iter) {
  if (length(errors) > max.iter)
    return(TRUE)
  
  lasts <- tail(errors, 2)
  
  if (abs(lasts[2] - lasts[1]) <= eps)
    return(TRUE)
...
run.backpropagation.experiment <- function()
{
  ...
  #######
  print(sum(bp$errors[-1])/bp$iters)
  plot(
    1:bp$iters,
    bp$errors[-1],
    type = "l",
